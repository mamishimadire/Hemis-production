...
namespace HemisAudit.Services
{
    ...
    public class Rule47Service : IRule47Service
    {
        ...
        {
            new Rule41ColumnPair { StudCol = "_001", H16Col = "_001", Label = "Qualification Code" },
            new Rule41ColumnPair { StudCol = "_002", H16Col = "_002", Label = "Type"               },
            new Rule41ColumnPair { StudCol = "_003", H16Col = "_003", Label = "Reference"          },
        };
        ...
        public async Task<Rule41TableDiscoveryResult> GetTablesAsync(int clientId)
        {
            try
            {
                ...
                return new Rule41TableDiscoveryResult
                {
                    ...
                    AutoH16Table = FindFirst(tables, ["H16QUAL", "H16_QUAL"], ["H16QUAL", "H16Q"])
                };
            }
...
-- ============================================================".Trim();
        ...
        private async Task<int> SaveValidationRunAsync(Rule41ValidationRequest request, Rule41ValidationSummary summary, string? userEmail, string? userName)
        {
            ...
            await _systemDb.MarkPreviousRuleRunsHistoricalAsync(request.ClientId, 47);

            var runId = await _systemDb.SaveValidationRunAsync(new SaveValidationRunRequest
            {
                ClientId = request.ClientId,
                RuleNumber = 47,
                RuleName = "QUAL vs H16QUAL Agreement",
        ...
        {
            var row = await _systemDb.GetCurrentRuleRunAsync(clientId, 47);
        ...
        {
            var row = await _systemDb.GetRuleRunByIdAsync(runId, 47);
            if (row == null) return null;
