...
{
    // Rule 32: Fatal Errors with Exclusions (STUD) — validates against the engagement's own
    ...
    // RowLimit is introduced here from the start, matching house style. This is the canonical
    // implementation shared (via ApplyRuleViewBag-parameterized views/controllers) by Rule28 (CESM),
    // Rule30 (PROF), and independently duplicated for Rule31 (QUAL) and Rule32 (STUD) itself.
    public class Rule32Service : IRule32Service
    {
        ...
        public async Task<TableListResult> GetTablesAsync(int clientId)
        {
            try
            {
                ...
                var autoTable = tables.FirstOrDefault(t => t.Equals("dbo_STUD_VALIDATION_DETAIL", StringComparison.OrdinalIgnoreCase))
                    ?? tables.FirstOrDefault(t => t.Equals("STUD_VALIDATION_DETAIL", StringComparison.OrdinalIgnoreCase))
                    ?? tables.FirstOrDefault(t => t.Equals("dbo_CESM_VALIDATION_DETAIL", StringComparison.OrdinalIgnoreCase))
                    ?? tables.FirstOrDefault(t => t.EndsWith("STUD_VALIDATION_DETAIL", StringComparison.OrdinalIgnoreCase))
                    ?? tables.FirstOrDefault(t => t.Contains("STUD_VALIDATION_DETAIL", StringComparison.OrdinalIgnoreCase));
...
WHERE UPPER(TRIM(CAST(""{request.ErrorTypeColumn}"" AS text))) = UPPER(@filterValue);";
        ...
        {
            var row = await _systemDb.GetCurrentRuleRunAsync(clientId, 32);
        ...
        {
            var row = await _systemDb.GetRuleRunByIdAsync(runId, 32);
            if (row == null) return null;
        ...
        public async Task AddOrUpdateSignoffAsync(int runId, string reviewerEmail, string comment)
        {
            ...
            var clientId = await _systemDb.GetClientIdForRunAsync(runId)
                ?? throw new InvalidOperationException("The selected Rule 32 run could not be found.");
            ...
            if (!CanSignOffAsRole(signoffRole))
                throw new InvalidOperationException("Only the assigned data analyst, manager, or director can sign off a Rule 32 run.");
...
DROP TABLE IF EXISTS rule32_classified;
CREATE TEMP TABLE rule32_classified AS
SELECT
...
WHERE UPPER(TRIM(CAST(""{request.ErrorTypeColumn}"" AS text))) = UPPER(@filterValue);";
        ...
        {
            await _systemDb.MarkPreviousRuleRunsHistoricalAsync(request.ClientId, 32);

            var runId = await _systemDb.SaveValidationRunAsync(new SaveValidationRunRequest
            {
                ClientId = request.ClientId,
                RuleNumber = 32,
                RuleName = "Fatal Errors with Exclusions (STUD)",
