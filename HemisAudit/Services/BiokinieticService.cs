...
FROM results;";
        ...
        public async Task<BiokinieticValidationSummary> GetExportSummaryAsync(BiokinieticValidationRequest request)
            => await AnalyseAsync(request, rowLimit: null);

        public async Task<int> GetPopulationCountAsync(BiokinieticValidationRequest request)
        {
            await ValidateColumnsExistAsync(request.ClientId, request.BiokinieticTable, [request.QualificationColumn, request.SurnameColumn]);
            await ValidateColumnsExistAsync(request.ClientId, request.ProductionTable, [request.QualificationColumn]);
        ...
        private async Task<BiokinieticValidationSummary> AnalyseAsync(BiokinieticValidationRequest request, int? rowLimit)
        {
            await ValidateColumnsExistAsync(request.ClientId, request.BiokinieticTable, [request.QualificationColumn, request.SurnameColumn]);
            await ValidateColumnsExistAsync(request.ClientId, request.ProductionTable, [request.QualificationColumn]);
