...
{
    // Rule 29: Single Column Filter — validates against the engagement's own uploaded Supabase
    // data instead of a live SQL Server connection. Structurally identical to Rule27 (single table
    // filtered by one column matching any of a set of entered values, with leading-zero numeric
    // normalization, 100% of matching rows retrieved with no sampling). MatchingRowLimit applied
    // from the start, matching Rule27's fix for the unbounded-load risk that caused Rule18's
    ...
    public class Rule29Service : IRule29Service
    {
        ...
        public async Task<TableListResult> GetTablesAsync(int clientId)
        {
            try
            {
                ...
                var autoTable = tables.FirstOrDefault(t => t.Equals("dbo_CREG_VALIDATION_DETAIL", StringComparison.OrdinalIgnoreCase))
                    ?? tables.FirstOrDefault(t => t.Equals("dbo_CESM_VALIDATION_DETAIL", StringComparison.OrdinalIgnoreCase))
        ...
        public async Task<Rule29ColumnSelectionResult> GetColumnsAsync(int clientId, string tableName)
        {
            try
            {
            ...
            {
                return new Rule29ColumnSelectionResult { Success = false, Error = ex.Message };
            }
        }

        public async Task<Rule29FilterValueResult> GetFilterValuesAsync(int clientId, string tableName, string filterColumn)
        {
            try
            {
...
LIMIT 20;";
            ...
            {
                return new Rule29FilterValueResult { Success = false, Error = ex.Message };
            }
        }
        ...
        public async Task<Rule29VerifyResult> VerifyTableAsync(Rule29VerifyRequest request)
        {
            ...
            {
                return new Rule29VerifyResult { Success = false, Error = ex.Message };
            }
        }
        ...
        public async Task<Rule29ValidationSummary> RunValidationAsync(Rule29ValidationRequest request, string? userEmail = null, string? userName = null)
        {
            ...
            {
                return new Rule29ValidationSummary { Success = false, Error = ex.Message };
            }
        }
        ...
        {
            var row = await _systemDb.GetCurrentRuleRunAsync(clientId, 29);
            ...
            var workspace = new Rule29WorkspaceStateViewModel
            {
                ...
                FilterValue = string.IsNullOrWhiteSpace(row.DeceasedColumn) ? "00708" : row.DeceasedColumn,
        ...
        {
            var row = await _systemDb.GetRuleRunByIdAsync(runId, 29);
            if (row == null) return null;

            var summary = DeserializeSummary(row.ResultsJSON);
            if (summary == null) return null;

            var review = new Rule29RunReviewViewModel
            {
                RunId = row.RunId,
                ClientId = row.ClientId,
        ...
        public async Task<Rule29WorkspaceSaveResult> SaveWorkspaceAsync(Rule29ValidationRequest request, string reviewerEmail, string? reviewerName = null)
        {
            try
            {
                if (!request.RunId.HasValue || request.RunId.Value <= 0)
                    return new Rule29WorkspaceSaveResult { Success = false, Error = "Run the filter first so the workspace can be saved." };

                var clientId = await _systemDb.GetClientIdForRunAsync(request.RunId.Value);
                if (!clientId.HasValue || clientId.Value != request.ClientId)
                    return new Rule29WorkspaceSaveResult { Success = false, Error = "The saved workspace could not be found for this engagement." };
            ...
            {
                return new Rule29WorkspaceSaveResult { Success = false, Error = ex.Message };
            }
        }
        ...
        public async Task<Rule29WorkspaceSaveResult> BeginWorkspaceEditAsync(int runId, string reviewerEmail, string? reviewerName = null)
        {
            try
            {
                ...
                if (!clientId.HasValue)
                    return new Rule29WorkspaceSaveResult { Success = false, Error = "Saved workspace was not found." };
        ...
        public async Task AddOrUpdateSignoffAsync(int runId, string reviewerEmail, string comment)
        {
            ...
            var clientId = await _systemDb.GetClientIdForRunAsync(runId)
                ?? throw new InvalidOperationException("The selected Rule 29 run could not be found.");
            ...
            if (!CanSignOffAsRole(signoffRole))
                throw new InvalidOperationException("Only the assigned data analyst, manager, or director can sign off a Rule 29 run.");
...
-- Breakdown Column: {(hasBreakdown ? $"\"{request.BreakdownColumn}\"" : "(not selected)")}
-- ============================================================================
...
";
            ...
            {
                sql += $@"
-- STEP 3: BREAKDOWN BY ""{request.BreakdownColumn}""
SELECT
    TRIM(CAST(""{request.BreakdownColumn}"" AS text)) AS breakdown_value,
    COUNT(*) AS record_count,
    CAST(COUNT(*) * 100.0 / NULLIF(SUM(COUNT(*)) OVER (), 0) AS decimal(10,2)) AS percentage
FROM ""{{schema}}"".""{request.TableName}""
WHERE {sqlFilterPredicate}
GROUP BY TRIM(CAST(""{request.BreakdownColumn}"" AS text))
...
FROM ""{{schema}}"".""{request.TableName}""
WHERE {sqlFilterPredicate}
{(showAll ? "" : $"ORDER BY random() LIMIT {sampleSize}")};
...
";
        ...
        private async Task<Rule29ValidationSummary> AnalyseAsync(Rule29ValidationRequest request)
        {
            ...
            if (hasBreakdown)
            {
                ...
                var breakdownPredicate = BuildFilterPredicate(breakdownCommand, request.FilterColumn, parsedValues, normalizedValues);
                breakdownCommand.CommandText = $@"
SELECT TRIM(CAST(""{request.BreakdownColumn}"" AS text)) AS breakdown_value, COUNT(*) AS record_count
FROM ""{schema}"".""{request.TableName}""
...
ORDER BY COUNT(*) DESC, breakdown_value ASC;";
            ...
            var effectiveLimit = request.ShowAllRecords ? MatchingRowLimit : Math.Min(sampleSize, MatchingRowLimit);
            ...
            if (matchingCount > 0)
            {
                ...
                dataCommand.Parameters.AddWithValue("limit", effectiveLimit);
            ...
            var matchingRowsTruncated = request.ShowAllRecords && matchingCount > matchingRows.Count;
        ...
        {
            await _systemDb.MarkPreviousRuleRunsHistoricalAsync(request.ClientId, 29);

            var runId = await _systemDb.SaveValidationRunAsync(new SaveValidationRunRequest
            {
                ClientId = request.ClientId,
                RuleNumber = 29,
                RuleName = "Single Column Filter",
