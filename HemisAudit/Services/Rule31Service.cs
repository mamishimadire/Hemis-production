...
{
    // Rule 31: Fatal Errors with Exclusions (QUAL) — validates against the engagement's own
    ...
    // the canonical Rule32Service (STUD) — Rule31 keeps its own ViewModels/views (unlike Rule28/30
    // which share Rule32's), but the service logic is otherwise the same.
    public class Rule31Service : IRule31Service
    {
        ...
        public async Task<TableListResult> GetTablesAsync(int clientId)
        {
            try
            {
                ...
                var autoTable = tables.FirstOrDefault(t => t.Equals("dbo_QUAL_VALIDATION_DETAIL", StringComparison.OrdinalIgnoreCase))
                    ?? tables.FirstOrDefault(t => t.Equals("QUAL_VALIDATION_DETAIL", StringComparison.OrdinalIgnoreCase))
                    ?? tables.FirstOrDefault(t => t.EndsWith("QUAL_VALIDATION_DETAIL", StringComparison.OrdinalIgnoreCase))
                    ?? tables.FirstOrDefault(t => t.Contains("QUAL_VALIDATION_DETAIL", StringComparison.OrdinalIgnoreCase));
        ...
        public async Task<Rule31ColumnSelectionResult> GetColumnsAsync(int clientId, string tableName)
        {
            try
            {
            ...
            {
                return new Rule31ColumnSelectionResult { Success = false, Error = ex.Message };
            }
        }

        public async Task<Rule31FilterValueResult> GetFilterValuesAsync(int clientId, string tableName, string errorTypeColumn)
        {
            try
            {
...
ORDER BY COUNT(*) DESC, filter_value ASC;";
            ...
            {
                return new Rule31FilterValueResult { Success = false, Error = ex.Message };
            }
        }
...
WHERE UPPER(TRIM(CAST(""{request.ErrorTypeColumn}"" AS text))) = UPPER(@filterValue);";
            ...
            {
                return new Rule31VerifyResult { Success = false, Error = ex.Message };
            }
        }
        ...
        public async Task<Rule31ValidationSummary> RunValidationAsync(Rule31ValidationRequest request, string? userEmail = null, string? userName = null)
        {
            ...
            {
                return new Rule31ValidationSummary { Success = false, Error = ex.Message };
            }
        }
        ...
        {
            var row = await _systemDb.GetCurrentRuleRunAsync(clientId, 31);
        ...
        {
            var row = await _systemDb.GetRuleRunByIdAsync(runId, 31);
            if (row == null) return null;

            var summary = DeserializeSummary(row.ResultsJSON);
            if (summary == null) return null;

            var review = new Rule31RunReviewViewModel
            {
                RunId = row.RunId,
                ClientId = row.ClientId,
        ...
        public async Task<Rule31WorkspaceSaveResult> SaveWorkspaceAsync(Rule31ValidationRequest request, string reviewerEmail, string? reviewerName = null)
        {
            try
            {
                if (request.RunId is null || request.RunId <= 0)
                    return new Rule31WorkspaceSaveResult { Success = false, Error = "Run validation before saving the workspace." };

                var clientId = await _systemDb.GetClientIdForRunAsync(request.RunId.Value);
                if (!clientId.HasValue || clientId.Value != request.ClientId)
                    return new Rule31WorkspaceSaveResult { Success = false, Error = "The saved workspace could not be found for this engagement." };
            ...
            {
                return new Rule31WorkspaceSaveResult { Success = false, Error = ex.Message };
            }
        }
        ...
        public async Task<Rule31WorkspaceSaveResult> BeginWorkspaceEditAsync(int runId, string reviewerEmail, string? reviewerName = null)
        {
            try
            {
                ...
                if (!clientId.HasValue)
                    return new Rule31WorkspaceSaveResult { Success = false, Error = "Saved workspace was not found." };
        ...
        public async Task AddOrUpdateSignoffAsync(int runId, string reviewerEmail, string comment)
        {
            ...
            var clientId = await _systemDb.GetClientIdForRunAsync(runId)
                ?? throw new InvalidOperationException("The selected Rule 31 run could not be found.");
            ...
            if (!CanSignOffAsRole(signoffRole))
                throw new InvalidOperationException("Only the assigned data analyst, manager, or director can sign off a Rule 31 run.");
...
DROP TABLE IF EXISTS rule31_classified;
CREATE TEMP TABLE rule31_classified AS
SELECT
...
WHERE UPPER(TRIM(CAST(""{request.ErrorTypeColumn}"" AS text))) = UPPER(@filterValue);";
        ...
        public async Task StreamCsvExportAsync(Rule31ValidationRequest request, bool onlyExceptions, Stream outputStream)
        {
            var exclusions = ParseExclusions(request.ExclusionCodes);
            var normalizedExclusions = exclusions.Select(NormalizeErrorCode).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
...
WHERE UPPER(TRIM(CAST(""{request.ErrorTypeColumn}"" AS text))) = UPPER(@filterValue);";
        ...
        {
            await _systemDb.MarkPreviousRuleRunsHistoricalAsync(request.ClientId, 31);

            var runId = await _systemDb.SaveValidationRunAsync(new SaveValidationRunRequest
            {
                ClientId = request.ClientId,
                RuleNumber = 31,
                RuleName = "Fatal Errors with Exclusions (QUAL)",
