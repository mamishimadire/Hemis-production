...
{
    // Rule 75: Mop — Qualification & Surname Validation — validates against the engagement's
    // own uploaded Supabase data instead of a live SQL Server connection. Population is every row in
    // the Mop source table with a non-blank qualification code. PASS when that qualification
    ...
    public class MopService : IMopService
    {
        ...
        public async Task<MopTableDiscoveryResult> GetTablesAsync(int clientId)
        {
            try
            {
        ...
        public async Task<MopColumnDiscoveryResult> GetColumnsAsync(int clientId, string tableName, string tableRole)
        {
            try
            {
                ...
                return new MopColumnDiscoveryResult { Success = true, Columns = cols, AutoSelected = auto };
            }
            catch (Exception ex) { return new MopColumnDiscoveryResult { Success = false, Error = ex.Message }; }
        }
        ...
        public async Task<MopVerifyResult> VerifyTablesAsync(MopVerifyRequest request)
        {
            try
            {
                await ValidateColumnsExistAsync(request.ClientId, request.MopTable, [request.QualificationColumn, request.SurnameColumn]);
                await ValidateColumnsExistAsync(request.ClientId, request.ProductionTable, [request.QualificationColumn]);

                var (conn, schema) = await OpenEngagementConnectionAsync(request.ClientId);
                await using var connection = conn;

                var srcTable = Sanitise(request.MopTable);
...
FROM results;";
                ...
                var result = new MopVerifyResult { Success = true, MopRecordCount = srcCount, ProductionRecordCount = prodCount };
        ...
        public async Task<MopValidationSummary> GetExportSummaryAsync(MopValidationRequest request)
            => await AnalyseAsync(request, rowLimit: null);

        public async Task<int> GetPopulationCountAsync(MopValidationRequest request)
        {
            await ValidateColumnsExistAsync(request.ClientId, request.MopTable, [request.QualificationColumn, request.SurnameColumn]);
            await ValidateColumnsExistAsync(request.ClientId, request.ProductionTable, [request.QualificationColumn]);
            ...
            var ctes = BuildValidationCtes(schema, request.MopTable, request.ProductionTable, qualCol, surnameCol);
        ...
        private async Task<MopValidationSummary> AnalyseAsync(MopValidationRequest request, int? rowLimit)
        {
            await ValidateColumnsExistAsync(request.ClientId, request.MopTable, [request.QualificationColumn, request.SurnameColumn]);
            await ValidateColumnsExistAsync(request.ClientId, request.ProductionTable, [request.QualificationColumn]);
            ...
            var srcCount = await CountAsync(connection, $"SELECT COUNT(*) FROM \"{schema}\".\"{Sanitise(request.MopTable)}\";");
...
FROM results;";
            ...
            var rows = new List<MopValidationRowRecord>();
            await using (var cmd = connection.CreateCommand())
            {
                cmd.CommandText = $@"
WITH {ctes}
...
{(rowLimit.HasValue ? "LIMIT @limit" : "")};";
                ...
                {
                    var row = new MopValidationRowRecord
                    {
            ...
            var controlSummaries = new List<MopControlSummaryItemViewModel>
            {
                new()
                {
                    ControlType  = "Control_1",
                    ControlLabel = "Control 1",
                    CriteriaText = $"Every {request.MopTable} [{qualCol}] value must exist in {request.ProductionTable} [{qualCol}]",
                    TotalCount   = total,
                    PassCount    = passed,
                    FailCount    = failed,
            ...
            return new MopValidationSummary
            {
                ...
                MopTable              = request.MopTable,
                ProductionTable       = request.ProductionTable,
                QualificationColumn   = qualCol,
                SurnameColumn         = surnameCol,
                TableLinkageText      = $"{request.MopTable}.{qualCol} <> {request.ProductionTable}.{qualCol}",
                RuleModeText          = $"100% population testing of {request.MopTable} against {request.ProductionTable}",
                ProcedureSteps        = new List<string>
                {
                    $"Select all records from {request.MopTable} where [{qualCol}] is not null/empty as the population to test.",
        ...
        private static void EnrichDisplayValues(MopValidationRowRecord row)
        {
        ...
        private static MopValidationSummary ApplyUiSample(MopValidationSummary full)
        {
            if (full.ReviewRows.Count <= UiSampleLimit) return full;
            return new MopValidationSummary
            {
                Success = full.Success, MopRecordCount = full.MopRecordCount, ProductionRecordCount = full.ProductionRecordCount,
                ...
                MopTable = full.MopTable, ProductionTable = full.ProductionTable,
        ...
        private static string BuildValidationCtes(string schema, string mopTable, string productionTable, string qualCol, string surnameCol)
        {
            var srcTable = Sanitise(mopTable);
            var prodTable = Sanitise(productionTable);

            return $@"
results AS (
    SELECT
        TRIM(CAST(mp.""{qualCol}"" AS text)) AS source_qual,
        TRIM(CAST(mp.""{surnameCol}"" AS text)) AS source_surname,
        (
            SELECT TRIM(CAST(p.""{surnameCol}"" AS text))
            FROM ""{schema}"".""{prodTable}"" p
            WHERE UPPER(TRIM(CAST(p.""{qualCol}"" AS text))) = UPPER(TRIM(CAST(mp.""{qualCol}"" AS text)))
            LIMIT 1
        ) AS matched_surname,
        CASE
            WHEN EXISTS (
    ...
    FROM ""{schema}"".""{srcTable}"" mp
    WHERE mp.""{qualCol}"" IS NOT NULL AND TRIM(CAST(mp.""{qualCol}"" AS text)) <> ''
)";
        }

        public string GenerateSql(MopValidationRequest request)
        {
            var qualCol = Sanitise(request.QualificationColumn);
            var surnameCol = Sanitise(request.SurnameColumn);
            var ctes = BuildValidationCtes("{schema}", request.MopTable, request.ProductionTable, qualCol, surnameCol);
            ...
            return $@"-- ============================================================
-- HEMIS RULE 75 - Mop Qualification & Surname Validation
-- Generated : {DateTime.Now:yyyy-MM-dd HH:mm:ss}
-- Source    : this engagement's own uploaded tables (schema engagement_{{ClientId}}), not a live SQL Server.
-- Check     : all {Sanitise(request.MopTable)}.[{qualCol}] values must exist in {Sanitise(request.ProductionTable)}.[{qualCol}]
-- ============================================================
WITH {ctes}
SELECT * FROM results
ORDER BY CASE WHEN validation_result = 'FAIL' THEN 0 ELSE 1 END, source_qual;".Trim();
        ...
        private async Task<int> SaveValidationRunAsync(MopValidationRequest request, MopValidationSummary summary, string? userEmail, string? userName)
        {
            ...
            await _systemDb.MarkPreviousRuleRunsHistoricalAsync(request.ClientId, 75);

            var runId = await _systemDb.SaveValidationRunAsync(new SaveValidationRunRequest
            {
                ClientId = request.ClientId,
                RuleNumber = 75,
                RuleName = "Mop Qualification & Surname Validation",
                Status = summary.Status,
                TotalRecords = summary.TotalValidated,
                PassCount = summary.PassCount,
                FailCount = summary.FailCount,
                ExceptionRate = summary.ExceptionRate,
                StudTable = request.MopTable,
        ...
        public async Task<MopValidationSummary?> GetStoredSummaryAsync(int runId)
        {
            var row = await _systemDb.GetRuleRunByIdAsync(runId, 75);
        ...
        public async Task<MopWorkspaceStateViewModel?> GetCurrentWorkspaceStateAsync(int clientId, string? currentUserEmail = null)
        {
            var row = await _systemDb.GetCurrentRuleRunAsync(clientId, 75);
            if (row == null) return null;

            var summary = DeserializeSummary(row.ResultsJSON);
            if (summary != null) summary = ApplyUiSample(summary);

            var workspace = new MopWorkspaceStateViewModel
            {
                ClientId = row.ClientId,
                RunId = row.RunId,
                MopTable = string.IsNullOrWhiteSpace(row.StudTable) ? "Mop" : row.StudTable,
        ...
        public async Task<MopRunReviewViewModel?> GetSavedRunAsync(int runId, string? currentUserEmail = null)
        {
            var row = await _systemDb.GetRuleRunByIdAsync(runId, 75);
            if (row == null) return null;
            ...
            var viewModel = new MopRunReviewViewModel
            {
                RunId = row.RunId,
                ClientId = row.ClientId,
            ...
            viewModel.GeneratedSql = GenerateSql(new MopValidationRequest
            {
        ...
        public async Task<MopWorkspaceSaveResult> SaveWorkspaceAsync(MopValidationRequest request, string reviewerEmail, string? reviewerName = null)
        {
            try
            {
                if (request.RunId is null || request.RunId <= 0)
                    return new MopWorkspaceSaveResult { Success = false, Error = "Run the validation first." };

                var clientId = await _systemDb.GetClientIdForRunAsync(request.RunId.Value);
                if (!clientId.HasValue || clientId.Value != request.ClientId)
                    return new MopWorkspaceSaveResult { Success = false, Error = "The saved run could not be found for this engagement." };
                ...
                return new MopWorkspaceSaveResult
                {
                    Success = true,
            ...
            catch (Exception ex) { return new MopWorkspaceSaveResult { Success = false, Error = ex.Message }; }
        }

        public async Task<MopWorkspaceSaveResult> BeginWorkspaceEditAsync(int runId, string reviewerEmail, string? reviewerName = null)
        {
            try
            {
                var clientId = await _systemDb.GetClientIdForRunAsync(runId);
                if (!clientId.HasValue)
                    return new MopWorkspaceSaveResult { Success = false, Error = "Saved run not found." };
        ...
        private static MopValidationSummary? DeserializeSummary(string? json)
        {
            if (string.IsNullOrWhiteSpace(json)) return null;
            try { return JsonConvert.DeserializeObject<MopValidationSummary>(ValidationPayloadCodec.Decode(json)); }
            catch { return null; }
        }
