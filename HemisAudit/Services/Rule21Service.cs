...
{
    // Rule 21: First-Time Entering Students vs Non-Aligned Qualifications — validates against
    // the engagement's own uploaded Supabase data instead of a live SQL Server connection.
    // Ported from the Rule19/Rule20 pattern. STUD is filtered on the first-time entering flag,
    ...
    public class Rule21Service : IRule21Service
    {
        ...
        public async Task<Rule21TableDiscoveryResult> GetTablesAsync(int clientId)
        {
            try
            {
        ...
        public async Task<Rule21ColumnDiscoveryResult> GetColumnsAsync(int clientId, string tableName)
        {
            try
            {
                var columns = await _datasets.GetValidatedColumnsAsync(clientId, tableName);
                return new Rule21ColumnDiscoveryResult
                {
                    Success = true,
                    Columns = columns,
        ...
        public async Task<Rule21DistinctValuesResult> GetDistinctValuesAsync(int clientId, string tableName, string columnName, string? preferredValue)
        {
            try
            {
        ...
        {
            var row = await _systemDb.GetCurrentRuleRunAsync(clientId, 21);
        ...
        {
            var row = await _systemDb.GetRuleRunByIdAsync(runId, 21);
            if (row == null) return null;
            ...
            var summary = DeserializeSummary(row.ResultsJSON) ?? new Rule21ValidationSummary();
            summary.ClientId = row.ClientId;
            if (summary.SavedRunId.GetValueOrDefault() <= 0)
                summary.SavedRunId = runId;

            var review = new Rule21RunReviewViewModel
            {
                RunId = row.RunId,
                ClientId = row.ClientId,
        ...
        public async Task AddOrUpdateSignoffAsync(int runId, string reviewerEmail, string comment)
        {
            ...
            var clientId = await _systemDb.GetClientIdForRunAsync(runId)
                ?? throw new InvalidOperationException("The selected Rule 21 run could not be found.");
            ...
            if (!CanSignOffAsRole(signoffRole))
                throw new InvalidOperationException("Only the assigned data analyst, manager, or director can sign off a Rule 21 run.");
        ...
        public async Task<string> GenerateSqlAsync(Rule21ValidationRequest request)
        {
            var cfg = await ResolveColumnConfigAsync(request);

            var sql = $@"-- HEMIS RULE 21: FIRST-TIME ENTERING STUDENTS VS NON-ALIGNED QUALIFICATIONS
...
{BuildRule21PrepSql("{schema}", request.StudTable, request.QualTable, request.NalTable, cfg)}

-- Full population (FLAGGED + CLEAR)
SELECT * FROM rule21_population ORDER BY row_number;
...
FROM rule21_population;";
        ...
        {
            var cfg = await ResolveColumnConfigAsync(request);
            await EnsureRule21IndexesAsync(request.ClientId, request.StudTable, request.QualTable, request.NalTable, cfg);
            ...
            {
                prepCommand.CommandText = BuildRule21PrepSql(schema, request.StudTable, request.QualTable, request.NalTable, cfg);
                await prepCommand.ExecuteNonQueryAsync();
            }

            var studFiltered = await CountAsync(connection, "SELECT COUNT(*) FROM rule21_population;");
        ...
        public async Task<Rule21ValidationSummary> GetExportSummaryAsync(Rule21ValidationRequest request) =>
        ...
        // Rule12Service.GetPopulationCountAsync.
        public async Task<int> GetPopulationCountAsync(Rule21ValidationRequest request)
        {
            var cfg = await ResolveColumnConfigAsync(request);
            await EnsureRule21IndexesAsync(request.ClientId, request.StudTable, request.QualTable, request.NalTable, cfg);
        ...
        public async Task StreamCsvExportAsync(Rule21ValidationRequest request, bool onlyExceptions, Stream outputStream)
        {
            var cfg = await ResolveColumnConfigAsync(request);
            await EnsureRule21IndexesAsync(request.ClientId, request.StudTable, request.QualTable, request.NalTable, cfg);
            ...
            command.CommandText = onlyExceptions
                ? "SELECT * FROM rule21_population WHERE result = 'FLAGGED' ORDER BY row_number;"
                : $@"
SELECT * FROM ( SELECT * FROM rule21_population WHERE result = 'FLAGGED' ORDER BY row_number ) flagged
UNION ALL
SELECT * FROM ( SELECT * FROM rule21_population WHERE result = 'CLEAR' ORDER BY row_number LIMIT {ClearSampleSize} ) clear_sample;";
        ...
        private static string BuildRule21PrepSql(string schema, string studTable, string qualTable, string nalTable, Rule21ColumnConfig cfg)
        {
            ...
            return $@"
DROP TABLE IF EXISTS rule21_qual;
CREATE TEMP TABLE rule21_qual AS
...
ORDER BY norm_code;
CREATE INDEX ON rule21_qual(norm_code);
ANALYZE rule21_qual;

DROP TABLE IF EXISTS rule21_nal;
CREATE TEMP TABLE rule21_nal AS
...
ORDER BY norm_ref;
CREATE INDEX ON rule21_nal(norm_ref);
ANALYZE rule21_nal;

DROP TABLE IF EXISTS rule21_population;
CREATE TEMP TABLE rule21_population AS
SELECT
...
LEFT JOIN rule21_qual q ON UPPER(TRIM(CAST(s.""{cfg.StudQualRefColumn}"" AS text))) = q.norm_code
LEFT JOIN rule21_nal n ON COALESCE(q.norm_code, UPPER(TRIM(CAST(s.""{cfg.StudQualRefColumn}"" AS text)))) = n.norm_ref
WHERE UPPER(TRIM(CAST(s.""{cfg.StudFirstTimeColumn}"" AS text))) = '{ftv}';

CREATE INDEX ON rule21_population(result);
ANALYZE rule21_population;";
        }
...
FROM rule21_population;";
        ...
        private static async Task<List<Rule21ValidationRowViewModel>> LoadRowsWhereAsync(NpgsqlConnection connection, string result, int limit)
        {
            await using var command = connection.CreateCommand();
            command.CommandText = "SELECT * FROM rule21_population WHERE result = @result ORDER BY row_number LIMIT @limit;";
            command.Parameters.AddWithValue("result", result);
            command.Parameters.AddWithValue("limit", limit);

            await using var reader = await command.ExecuteReaderAsync();
            var rows = new List<Rule21ValidationRowViewModel>();
            while (await reader.ReadAsync())
            {
                ...
                rows.Add(new Rule21ValidationRowViewModel
                {
        ...
        {
            await _systemDb.MarkPreviousRuleRunsHistoricalAsync(request.ClientId, 21);
        ...
        private sealed class Rule21ColumnConfig
        {
        ...
        private async Task<Rule21ColumnConfig> ResolveColumnConfigAsync(Rule21ValidationRequest request)
        {
            var cfg = new Rule21ColumnConfig
            {
        ...
        private async Task EnsureRule21IndexesAsync(int clientId, string studTable, string qualTable, string nalTable, Rule21ColumnConfig cfg)
        {
