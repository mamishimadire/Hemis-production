...
namespace HemisAudit.Controllers
{
    ...
    public class Rule34Controller : Controller
    {
        ...
        public async Task<IActionResult> DownloadSavedExcel(int runId)
        {
            ...
            var populationCount = await _rule34.GetPopulationCountAsync(exportRequest);
            ...
            var resolved = await _rule34.GetExportSummaryAsync(exportRequest);
        ...
        public async Task<IActionResult> DownloadSavedCsv(int runId)
        {
            ...
            var resolved = await _rule34.GetExportSummaryAsync(BuildRequestFromSummary(review.ClientId, review.Summary));
            var bytes = _export.ExportCsv(resolved, false);
        ...
        public async Task<IActionResult> DownloadSavedExceptionsCsv(int runId)
        {
            ...
            var resolved = await _rule34.GetExportSummaryAsync(BuildRequestFromSummary(review.ClientId, review.Summary));
            var bytes = _export.ExportCsv(resolved, true);
        ...
        public async Task<IActionResult> GetExportInfo(int runId)
        {
            ...
            var populationCount = await _rule34.GetPopulationCountAsync(BuildRequestFromSummary(review.ClientId, review.Summary));
            return Json(new
            {
        ...
        private static Rule34ValidationRequest BuildRequestFromSummary(int clientId, Rule34ValidationSummary s) => new()
        {
            ClientId = clientId,
            TableName = s.TableName,
            ClientTableName = s.ClientTableName,
            FirstDayColumn = s.FirstDayColumn,
            LastDayColumn = s.LastDayColumn,
            CensusDateColumn = s.CensusDateColumn,
            ClientJoinColumn = s.ClientJoinColumn,
            StartYear = s.StartYear,
            EndYear = s.EndYear,
            BlockColumn = s.BlockColumn,
            BlockExcludeValues = s.BlockExcludeValues
        };
        ...
        [HttpPost]
        public async Task<IActionResult> GetExportInfo([FromBody] Rule34ValidationSummary summary)
        {
            try
            {
                var exportRequest = await ResolveExportRequestAsync(summary);
        ...
        private async Task<Rule34ValidationRequest> ResolveExportRequestAsync(Rule34ValidationSummary summary)
        {
            if (summary == null)
                throw new InvalidOperationException("Run Rule 34 first before downloading results.");
            ...
            if (string.IsNullOrWhiteSpace(summary.TableName))
                throw new InvalidOperationException("Run Rule 34 first before downloading results.");
