...
namespace HemisAudit.Controllers
{
    ...
    public class Rule16Controller : Controller
    {
        ...
        public async Task<IActionResult> GetTables([FromBody] EngagementTableListRequest model) =>
            Json(await RequireDataAnalystAsync(async () => await _rule16.GetTablesAsync(model.ClientId)));

        [HttpPost]
        public async Task<IActionResult> GetColumns([FromBody] Rule16ColumnsRequest request) =>
            Json(await RequireDataAnalystAsync(async () => await _rule16.GetColumnsAsync(request.ClientId, request.TableName)));
        ...
        public async Task<IActionResult> DownloadExcel([FromBody] Rule16ValidationSummary summary)
        {
            try
            {
                ...
                var populationCount = await _rule16.GetPopulationCountAsync(exportRequest);
                ...
                var resolved = await _rule16.GetExportSummaryAsync(exportRequest);
        ...
        [HttpPost]
        public async Task<IActionResult> GetExportInfo([FromBody] Rule16ValidationSummary summary)
        {
            try
            {
                var exportRequest = await ResolveExportRequestAsync(summary);
        ...
        public async Task<IActionResult> DownloadCsv([FromBody] Rule16ValidationSummary summary)
        {
            try
            {
                ...
                Response.Headers.ContentDisposition = $"attachment; filename=\"Rule16_Student_Population_{Ts()}.csv\"";
                await _rule16.StreamCsvExportAsync(exportRequest, Response.Body);
        ...
        private async Task<Rule16ValidationRequest> ResolveExportRequestAsync(Rule16ValidationSummary summary)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            if (string.IsNullOrWhiteSpace(summary.StudTable) || string.IsNullOrWhiteSpace(summary.BridgeTable) || string.IsNullOrWhiteSpace(summary.CrseTable))
                throw new InvalidOperationException("Run Rule 16 first before downloading results.");

            return new Rule16ValidationRequest
            {
