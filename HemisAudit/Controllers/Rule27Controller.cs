...
namespace HemisAudit.Controllers
{
    ...
    public class Rule27Controller : Controller
    {
        ...
        public async Task<IActionResult> DownloadSavedExcel(int runId)
        {
            ...
            var populationCount = await _rule27.GetPopulationCountAsync(exportRequest);
            ...
            var resolved = await _rule27.GetExportSummaryAsync(exportRequest);
        ...
        public async Task<IActionResult> DownloadSavedCsv(int runId)
        {
            ...
            Response.Headers.ContentDisposition = $"attachment; filename=\"Rule27_{scope}_Run_{runId}.csv\"";
            await _rule27.StreamCsvExportAsync(exportRequest, Response.Body);
            return new EmptyResult();
        }

        private static Rule27ValidationRequest BuildRequestFromSummary(int clientId, Rule27ValidationSummary summary) => new()
        {
            ClientId = clientId,
        ...
        [HttpPost]
        public async Task<IActionResult> GetExportInfo([FromBody] Rule27ValidationSummary summary)
        {
            try
            {
                var exportRequest = await ResolveExportRequestAsync(summary);
        ...
        public async Task<IActionResult> DownloadCsv([FromBody] Rule27ValidationSummary summary)
        {
            try
            {
                ...
                Response.ContentType = "text/csv";
                Response.Headers.ContentDisposition = $"attachment; filename=\"Rule27_{scope}_{Ts()}.csv\"";
                await _rule27.StreamCsvExportAsync(exportRequest, Response.Body);
                return new EmptyResult();
            }
            catch (Exception ex)
        ...
        private async Task<Rule27ValidationRequest> ResolveExportRequestAsync(Rule27ValidationSummary summary)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            if (string.IsNullOrWhiteSpace(summary.TableName) || string.IsNullOrWhiteSpace(summary.FilterColumn))
                throw new InvalidOperationException("Run Rule 27 first before downloading results.");
