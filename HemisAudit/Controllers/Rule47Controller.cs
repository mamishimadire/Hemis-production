...
namespace HemisAudit.Controllers
{
    ...
    public class Rule47Controller : Controller
    {
        ...
        public async Task<IActionResult> GetTables([FromBody] EngagementTableListRequest model) =>
            Json(await RequireDataAnalystAsync(async () => await _rule47.GetTablesAsync(model.ClientId)));

        [HttpPost]
        public async Task<IActionResult> GetColumns([FromBody] Rule16ColumnsRequest model) =>
            Json(await RequireDataAnalystAsync(async () =>
                await _rule47.GetColumnsAsync(model.ClientId, model.TableName)));
        ...
        public async Task<IActionResult> Run(int id)
        {
            ...
            var role = await GetCurrentSystemRoleAsync(user);
            await _systemDb.NormalizeCompletedRunStatusesAsync();
            var review = await _rule47.GetSavedRunAsync(id, user?.Email);
            if (review == null)
                return NotFound();
            ...
            ViewBag.ModuleNavigation = ModuleSequenceNavigationHelper.BuildForSavedRun(47,
                review.ClientId,
            ...
            if (review.Summary != null)
                review.GeneratedSql = _rule47.GenerateSql(BuildRequestFromSummary(review.Summary));
        ...
        public async Task<IActionResult> DownloadExcel([FromQuery] int runId)
        {
            ...
            var populationCount = await _rule47.GetPopulationCountAsync(exportRequest);
            ...
            var resolved = await _rule47.GetExportSummaryAsync(exportRequest);
            var bytes = _export.ExportRule41FamilyExcel(resolved, "HEMIS RULE 47 - QUAL vs H16QUAL Agreement", "QUAL", "H16QUAL");
        ...
        public async Task<IActionResult> DownloadCsv([FromQuery] int runId)
        {
            ...
            if (review == null || !await _systemDb.CanAccessClientResultsAsync(review.ClientId, user, role))
                return NotFound();
            var resolved = await _rule47.GetExportSummaryAsync(BuildRequestFromSummary(review.Summary!));
        ...
        public async Task<IActionResult> GetExportInfo([FromQuery] int runId)
        {
            ...
            if (review == null || !await _systemDb.CanAccessClientResultsAsync(review.ClientId, user, role))
                return NotFound();
            var populationCount = await _rule47.GetPopulationCountAsync(BuildRequestFromSummary(review.Summary!));
            return Json(new
            {
        ...
        private static byte[] BuildCsvExport(Rule41ValidationSummary summary, bool exceptionsOnly)
        {
            ...
            sw.WriteLine($"\"HEMIS RULE 47 - {(exceptionsOnly ? "Exceptions" : "All Results")}\"");
