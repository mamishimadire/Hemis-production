...
public class ClinicalDraftReportController : Controller
{
    ...
    private static void BuildDocument(MemoryStream ms)
    {
        ...
        WordHelper.AddHeaderTable(body,
            ["The Chief Financial Officer (CFO)", "........ University", "Private Bag X680", "Pretoria", "0001"],
        ...
        body.Append(WordHelper.WPara(
            "Agreed-upon Procedures Report on Clinical Training Enrollment Audit for the academic year ended xx December xxxx.",
        ...
        WordHelper.AddSection(body, "Purpose of this Agreed-Upon Procedure Report and Restriction on Use and Distribution",
            "Our report is solely for the purpose of assisting ........ University in determining whether the Clinical Enrolment data submitted to the Department of Higher Education and Training (DHET) are compliant with the requirements of the DHET Clinical Training Enrolment policy and may not be suitable for any other purpose. This report is intended solely for the use of ........ University and the DHET, and should not be used by, or distributed to, any other parties.");

        WordHelper.AddSection(body, "Responsibility of the Engaging Party and the Responsible Party",
            "........ University has acknowledged that the agreed-upon procedures are appropriate for the purpose of the engagement.\n\nThe Acting DVC: Digital Transformation, as identified by ........ University, is responsible for the subject matter on which the agreed-upon procedures are performed");
        ...
        tbl.Append(WordHelper.ProcDataRow("1",
            "Obtain from Statutory Reporting: Student and Space in the Strategic Management Support Department of the University, the Clinical Training Student WIL List for the relevant academic year from the University's official student records.",
            ("We have obtained from Statutory Reporting: Student and Space in the Strategic Management Support Department, the Clinical Training Student WIL List for the relevant academic year from the University's official student records.", false, (string?)null),
            ("", false, null),
        ...
        tbl.Append(WordHelper.ProcDataRow("2.1",
            "Obtain from Statutory Reporting: Student and Space in the Strategic Management Support Department, the Clinical Training HEMIS Headcounts enrolment data for the relevant academic year from the University’s official student record system (“HEMIS”).",
            ("We have obtained from Statutory Reporting: Student and Space in the Strategic Management Support Department, the Clinical Training HEMIS Headcounts enrolment data for the relevant academic year from the University’s official student record system (“HEMIS”).", false, null),
            ("", false, null),
            ("No exceptions noted.", true, WordHelper.Purple)));
        ...
        tbl.Append(WordHelper.ProcDataRow("3",
            "Select a sample of students for each area of study for the relevant academic year. The sample should only be selected from student criteria that contain curriculum-stated Work Integrated Learning (WIL) periods contained in procedure 1 above.",
            ("We have selected a sample of 5 students for each area of study for the relevant academic year. The sample was only selected from student criteria that contain curriculum-stated Work Integrated Learning (WIL) periods contained in procedure 1 above.", false, null),
        ...
        tbl.Append(WordHelper.ProcDataRow("4",
            "Inspect evidence obtained from the Deputy-Vice Chancellor: Teaching, Learning and Technology indicating that students selected in procedure 3 were active students at the University in the relevant academic year (i.e. inspect logbooks, hour sheets, workbooks, portfolio of evidence, and proof of registration/academic records).",
            ("Inspected evidence obtained from the Deputy-Vice Chancellor: Teaching, Learning and Technology indicating that students selected in procedure 3 were active students at the University in the relevant academic year (i.e. inspect logbooks, hour sheets, workbooks, portfolio of evidence, and proof of registration/academic records).", false, null),
            ("", false, null),
            ("No exceptions noted.", true, WordHelper.Purple)));
        ...
        tbl.Append(WordHelper.ProcDataRow("6",
            ...
            ("Inspected the approved relevant University Prospectus obtained from the Deputy-Vice Chancellor: Teaching, Learning and Technology and confirmed that the Health Science programmes are accredited and offer training within health science disciplines.", false, null),
            ("", false, null),
            ("No exceptions noted.", true, WordHelper.Purple)));

        tbl.Append(WordHelper.ProcDataRow("7",
            "Inspect the approved relevant University Prospectus obtained from the Deputy-Vice Chancellor: Teaching, Learning and Technology that the curriculum of the health sciences programme includes clinical training which requires students to have access to the facilities, patients, and clinical staff of provincial health care services. (Sample selected in accordance with Procedure 3).",
        ...
        tbl.Append(WordHelper.ProcDataRow("8",
            "For the undergraduate level, inspect the approved relevant University Prospectus that only the health sciences programmes which offer initial training in a health sciences discipline are included in the schedule.",
            ("For the undergraduate level, inspected the approved relevant University Prospectus that only the health sciences programmes which offer initial training in a health sciences discipline are included in the schedule.", false, null),
            ("", false, null),
            ("No exceptions noted.", true, WordHelper.Purple)));
        ...
        tbl.Append(WordHelper.ProcDataRow("10",
            "Agree the head counts enrolment in the DHET reporting template (relevant reporting period Head Count Enrolments By Clinical Programme) received from the University to the Clinical Training HEMIS Headcounts enrolment data.",
            ("Agreed the head counts enrolment of 1 271 in the DHET reporting template (relevant reporting period Head Count Enrolments By Clinical Programme) received from the University to the Clinical Training HEMIS Headcounts enrolment data of 1 271.", false, null),
            ("", false, null),
            ("No exceptions noted.", true, WordHelper.Purple)));
        ...
        body.Append(WordHelper.WPara("Address line 1", afterPt: 0));
        body.Append(WordHelper.WPara("Address line 2", afterPt: 0));
