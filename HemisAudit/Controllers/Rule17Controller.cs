...
namespace HemisAudit.Controllers
{
    ...
    public class Rule17Controller : Controller
    {
        ...
        public async Task<IActionResult> DownloadSavedExcel(int runId)
        {
            ...
            var populationCount = await _rule17.GetPopulationCountAsync(BuildRequestFromSummary(review.ClientId, review.Summary));
        ...
        public async Task<IActionResult> DownloadExcel([FromBody] Rule17ValidationSummary summary)
        {
            ...
            var populationCount = await _rule17.GetPopulationCountAsync(BuildRequestFromSummary(summary.ClientId, summary));
            if (populationCount > ExcelExportRowSafetyLimit)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> GetExportInfo([FromBody] Rule17ValidationSummary summary)
        {
            var resolved = await ResolveExportSummaryAsync(summary);
            var populationCount = await _rule17.GetPopulationCountAsync(BuildRequestFromSummary(resolved.ClientId, resolved));
            return Json(new
            {
        ...
        private static Rule17ValidationRequest BuildRequestFromSummary(int clientId, Rule17ValidationSummary s) => new()
        {
            ClientId = clientId,
            TableName = s.TableName,
            StudJoinCol = s.StudJoinCol,
            QualTable = s.QualTable,
            QualJoinCol = s.QualJoinCol,
            QualNameCol = s.QualNameCol,
            FilterColumn = s.FilterColumn,
            FilterValue = s.FilterValue,
            BreakdownColumn = s.BreakdownColumn,
            SampleSize = s.SampleSize,
            ShowAllRecords = s.ShowAllRecords
        };
