...
namespace HemisAudit.Controllers
{
    ...
    public class Rule59Controller : Controller
    {
        ...
        public async Task<IActionResult> DownloadSavedExcel(int runId)
        {
            ...
            var populationCount = await _rule59.GetPopulationCountAsync(exportRequest);
            ...
            var resolved = await _rule59.GetExportSummaryAsync(exportRequest);
            return File(_export.ExportRule59Excel(resolved), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", $"Rule59_SFTE_VALPAC_Production_Run_{runId}.xlsx");
        }
        ...
        public async Task<IActionResult> DownloadSavedCsv(int runId)
        {
            ...
            var exportRequest = BuildRequestFromSummary(review.ClientId, review.Summary!);
            var resolved = await _rule59.GetExportSummaryAsync(exportRequest);
            return File(_export.ExportRule59Csv(resolved), "text/csv", $"Rule59_SFTE_VALPAC_Production_Run_{runId}.csv");
        }
        ...
        [HttpPost]
        public async Task<IActionResult> GetExportInfo([FromBody] Rule59ValidationSummary summary)
        {
            try
            {
                var exportRequest = await ResolveExportRequestAsync(summary);
        ...
        private async Task<Rule59ValidationRequest> ResolveExportRequestAsync(Rule59ValidationSummary summary)
        {
            if (summary == null)
                throw new InvalidOperationException("Run Rule 59 first before downloading results.");
