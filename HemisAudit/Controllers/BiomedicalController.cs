...
namespace HemisAudit.Controllers
{
    ...
    public class BiomedicalController : Controller
    {
        ...
        public BiomedicalController(IBiomedicalService biomedical, IExportService export, IAuditLogService audit, UserManager<ApplicationUser> users, ISystemDatabaseService systemDb)
        {
            _biomedical = biomedical; _export = export; _audit = audit; _users = users; _systemDb = systemDb;
        }
        ...
        public async Task<IActionResult> Run(int id)
        {
            ...
            var review = await _biomedical.GetSavedRunAsync(id, user?.Email);
            ...
            ViewBag.ModuleNavigation = ModuleSequenceNavigationHelper.BuildForSavedRun(74, review.ClientId, clientDetail?.ValidationRuns, role, review.CurrentUserEngagementRole);
        ...
        public async Task<IActionResult> GetTables([FromBody] EngagementTableListRequest model) =>
            Json(await RequireDataAnalystAsync(async () => await _biomedical.GetTablesAsync(model.ClientId)));

        [HttpPost]
        public async Task<IActionResult> GetColumns([FromBody] BiomedicalGetColumnsRequest model) =>
            Json(await RequireDataAnalystAsync(async () => await _biomedical.GetColumnsAsync(model.ClientId, model.TableName, model.TableRole)));

        [HttpPost]
        public async Task<IActionResult> VerifyTables([FromBody] BiomedicalVerifyRequest request) =>
            Json(await RequireDataAnalystAsync(async () => await _biomedical.VerifyTablesAsync(request)));
        ...
        public async Task<IActionResult> RunValidation([FromBody] BiomedicalValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            if (request.ClientId <= 0) return Json(new BiomedicalValidationSummary { Success = false, Error = "Select an approved engagement before running validation." });
            if (!await _systemDb.CanAccessClientResultsAsync(request.ClientId, user, role)) return Json(new BiomedicalValidationSummary { Success = false, Error = "You cannot access this engagement." });
            ...
            if (!string.Equals(role, "DataAnalyst", StringComparison.OrdinalIgnoreCase) || !string.Equals(engagementRole, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
                return Json(new BiomedicalValidationSummary { Success = false, Error = "Only the assigned data analyst can run this validation." });

            async Task<BiomedicalValidationSummary> Execute(IBiomedicalService svc, IAuditLogService auditSvc)
            {
                var result = await svc.RunValidationAsync(request, user?.Email, user?.FullName ?? user?.Email);
                if (result.Success)
                    await auditSvc.LogAsync("run_validation", $"Rule 74 (Biomedical) on client {request.ClientId}: {result.Status} ({result.FailCount} fail rows).", user?.Id, user?.Email);
                return result;
            }
            ...
            if (ValidationOperationHttpHelper.IsAsyncRequested(Request))
            {
                return ValidationOperationHttpHelper.Queue(this, HttpContext.RequestServices.GetRequiredService<IValidationOperationService>(),
                    ValidationOperationHttpHelper.ResolveOwnerKey(User), "Rule 74 validation",
                    async (sp, ct) => await Execute(sp.GetRequiredService<IBiomedicalService>(), sp.GetRequiredService<IAuditLogService>()));
            }

            return Json(await Execute(_biomedical, _audit));
        }

        [HttpPost]
        public async Task<IActionResult> BeginWorkspaceEdit([FromBody] BiomedicalValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            if (!await CanEditWorkspaceAsync(request.ClientId, user, role)) return Json(new BiomedicalWorkspaceSaveResult { Success = false, Error = "Only the assigned data analyst can edit a saved workspace." });
            if (!request.RunId.HasValue || request.RunId.Value <= 0) return Json(new BiomedicalWorkspaceSaveResult { Success = false, Error = "Select a saved run before editing." });
            var result = await _biomedical.BeginWorkspaceEditAsync(request.RunId.Value, user!.Email!, user.FullName);
            if (result.Success) await _audit.LogAsync("workspace_edit_started", $"DataAnalyst started editing Rule 74 run {request.RunId.Value}.", user?.Id, user?.Email);
            return Json(result);
        }
        ...
        public async Task<IActionResult> SaveWorkspace([FromBody] BiomedicalValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            if (!await CanEditWorkspaceAsync(request.ClientId, user, role)) return Json(new BiomedicalWorkspaceSaveResult { Success = false, Error = "Only the assigned data analyst can save a workspace." });
            var result = await _biomedical.SaveWorkspaceAsync(request, user!.Email!, user.FullName);
            if (result.Success) await _audit.LogAsync("save_validation_workspace", $"DataAnalyst saved Rule 74 workspace for client {request.ClientId}. Run: {result.Workspace?.RunId}", user?.Id, user?.Email);
            return Json(result);
        }

        [HttpPost]
        public async Task<IActionResult> SignOffWorkspace([FromBody] BiomedicalWorkspaceSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            var review = await _biomedical.GetSavedRunAsync(model.RunId.Value, user?.Email);
            ...
            try
            {
                await _biomedical.AddOrUpdateSignoffAsync(model.RunId.Value, user!.Email!, model.Comment);
                await _audit.LogAsync("signoff_validation_run", $"Rule 74 signoff saved for run {model.RunId.Value}", user.Id, user.Email);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> RemoveWorkspaceSignoff([FromBody] BiomedicalWorkspaceSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            try { await _biomedical.RemoveSignoffAsync(model.RunId.Value, user!.Email!); }
            catch (Exception ex) { return Json(new { success = false, error = ex.Message }); }

            var workspace = await _biomedical.GetCurrentWorkspaceStateAsync(model.ClientId, user?.Email);
            var resultsVisible = CanViewWorkspaceResults(role, workspace);
            if (workspace != null) workspace.ResultsVisible = resultsVisible;
            await _audit.LogAsync("remove_validation_signoff", $"{review.CurrentUserEngagementRole} removed signoff for Rule 74 run {model.RunId.Value}", user?.Id, user?.Email);
        ...
        public async Task<IActionResult> GenerateSql([FromBody] BiomedicalValidationRequest request)
        {
            ...
            if (request.ClientId > 0 && !await _systemDb.CanAccessClientResultsAsync(request.ClientId, user, role))
                return Json(new BiomedicalSqlResult { Success = false, Error = "You cannot access this engagement." });
            return Json(await RequireDataAnalystAsync(async () => new BiomedicalSqlResult { Success = true, Sql = _biomedical.GenerateSql(request) }));
        }

        [HttpPost, ValidateAntiForgeryToken]
        public async Task<IActionResult> AddSignoff(BiomedicalRunSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _biomedical.GetSavedRunAsync(model.RunId, user?.Email);
            if (review == null) return NotFound();
            var clientDetail = await _systemDb.GetClientDetailAsync(review.ClientId, user, role);
            ...
            await _audit.LogAsync("signoff_validation_run", $"{review.CurrentUserEngagementRole} signed off Rule 74 run {model.RunId}", user.Id, user.Email);
        ...
        public async Task<IActionResult> RemoveSignoff(int runId)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _biomedical.GetSavedRunAsync(runId, user?.Email);
            if (review == null) return NotFound();
            var clientDetail = await _systemDb.GetClientDetailAsync(review.ClientId, user, role);
            ...
            await _biomedical.RemoveSignoffAsync(runId, user!.Email!);
            await _audit.LogAsync("remove_validation_signoff", $"{review.CurrentUserEngagementRole} removed signoff for Rule 74 run {runId}", user?.Id, user?.Email);
        ...
        public async Task<IActionResult> DownloadSavedExcel(int runId)
        {
            ...
            var populationCount = await _biomedical.GetPopulationCountAsync(exportRequest);
            ...
            var fullSummary = await _biomedical.GetExportSummaryAsync(exportRequest);
            ...
            var bytes = _export.ExportQualSurnameExcel("Biomedical", 74, fullSummary.TotalValidated, fullSummary.PassCount, fullSummary.FailCount, fullSummary.ExceptionRate, fullSummary.Status ?? "", fullSummary.BiomedicalTable, fullSummary.ProductionTable, fullSummary.QualificationColumn, fullSummary.SurnameColumn, rows);
            return File(bytes, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", $"Rule74_Biomedical_Run_{runId}.xlsx");
        }
        ...
        public async Task<IActionResult> DownloadSavedCsv(int runId)
        {
            var review = await LoadAuthorizedSavedRunAsync(runId, requireDownloadAccess: true);
            if (review == null) return RedirectToAction(nameof(Run), new { id = runId });
            var fullSummary = await _biomedical.GetExportSummaryAsync(BuildRequestFromSummary(review.ClientId, review.Summary));
            var rows = (fullSummary.ReviewRows ?? new()).Select(r => (
                r.DisplayValues.TryGetValue("SOURCE_QUAL", out var q) ? q ?? "" : "",
                r.DisplayValues.TryGetValue("SOURCE_SURNAME", out var s) ? s ?? "" : "",
                r.ValidationResult,
                string.Equals(r.ValidationResult, "PASS", StringComparison.OrdinalIgnoreCase) ? (r.DisplayValues.TryGetValue("SOURCE_QUAL", out var pq) ? pq ?? "" : "") : "",
                r.DisplayValues.TryGetValue("PRODUCTION_SURNAME", out var ps) ? ps ?? "" : ""));
            return File(_export.ExportQualSurnameCsv(rows), "text/csv", $"Rule74_Biomedical_Run_{runId}.csv");
        }
        ...
        public async Task<IActionResult> GetExportInfo(int runId)
        {
            ...
            if (review == null) return NotFound();
            var populationCount = await _biomedical.GetPopulationCountAsync(BuildRequestFromSummary(review.ClientId, review.Summary));
            return Json(new
            {
        ...
        public async Task<IActionResult> DownloadSavedSql(int runId)
        {
            var review = await LoadAuthorizedSavedRunAsync(runId, requireDownloadAccess: true);
            if (review == null) return RedirectToAction(nameof(Run), new { id = runId });
            var sql = _biomedical.GenerateSql(BuildRequestFromSummary(review.ClientId, review.Summary));
            return File(_export.ExportSql(sql), "application/sql", $"Rule74_Biomedical_{runId}.sql");
        }

        // ─── Private helpers ─────────────────────────────────────────────────

        private static BiomedicalValidationRequest BuildRequestFromSummary(int clientId, BiomedicalValidationSummary summary) => new()
        {
            ClientId = clientId,
        ...
        private async Task<BiomedicalRunReviewViewModel?> LoadAuthorizedSavedRunAsync(int runId, bool requireDownloadAccess)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _biomedical.GetSavedRunAsync(runId, user?.Email);
        ...
        private static bool CanDownloadSavedRun(BiomedicalRunReviewViewModel review, string systemRole)
        ...
        private static bool CanViewSavedRun(BiomedicalRunReviewViewModel review, string systemRole)
            => ValidationRunAccessPolicy.CanViewSignedResults(systemRole, review.CurrentUserEngagementRole, review.HasDataAnalystSignoff);

        private static bool CanViewWorkspaceResults(string role, BiomedicalWorkspaceStateViewModel? workspace)
        {
