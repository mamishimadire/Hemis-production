...
namespace HemisAudit.Controllers
{
    ...
    public class Rule30Controller : Controller
    {
        ...
        public async Task<IActionResult> GetTables([FromBody] EngagementTableListRequest model) =>
            Json(await RequireDataAnalystAsync(async () => await _rule30.GetTablesAsync(model.ClientId)));

        [HttpPost]
        public async Task<IActionResult> GetColumns([FromBody] Rule32GetColumnsRequest model) =>
            Json(await RequireDataAnalystAsync(async () => await _rule30.GetColumnsAsync(model.ClientId, model.TableName)));
        ...
        public async Task<IActionResult> LoadFilterValues([FromBody] Rule32FilterValueRequest model) =>
            Json(await RequireDataAnalystAsync(async () =>
                await _rule30.GetFilterValuesAsync(model.ClientId, model.TableName, model.ErrorTypeColumn)));
        ...
        public async Task<IActionResult> DownloadSavedExcel(int runId)
        {
            ...
            var populationCount = await _rule30.GetPopulationCountAsync(exportRequest);
            ...
            var resolved = await _rule30.GetExportSummaryAsync(exportRequest);
        ...
        public async Task<IActionResult> DownloadSavedCsv(int runId)
        {
            ...
            Response.Headers.ContentDisposition = $"attachment; filename=\"Rule30_Excluded_and_Remaining_Run_{runId}.csv\"";
            await _rule30.StreamCsvExportAsync(exportRequest, onlyExceptions: false, Response.Body);
        ...
        public async Task<IActionResult> DownloadSavedExceptionsCsv(int runId)
        {
            ...
            var exportRequest = BuildRequestFromSummary(review.ClientId, review.Summary);
            Response.ContentType = "text/csv";
            Response.Headers.ContentDisposition = $"attachment; filename=\"Rule30_Remaining_Errors_Run_{runId}.csv\"";
            await _rule30.StreamCsvExportAsync(exportRequest, onlyExceptions: true, Response.Body);
            return new EmptyResult();
        }
        ...
        public async Task<IActionResult> DownloadCsv([FromBody] Rule32ValidationSummary summary)
        {
            try
            {
                ...
                Response.ContentType = "text/csv";
                Response.Headers.ContentDisposition = $"attachment; filename=\"Rule30_Excluded_and_Remaining_{Ts()}.csv\"";
                await _rule30.StreamCsvExportAsync(exportRequest, onlyExceptions: false, Response.Body);
                return new EmptyResult();
            }
            catch (Exception ex)
        ...
        public async Task<IActionResult> DownloadExceptionsCsv([FromBody] Rule32ValidationSummary summary)
        {
            try
            {
                ...
                Response.ContentType = "text/csv";
                Response.Headers.ContentDisposition = $"attachment; filename=\"Rule30_Remaining_Errors_{Ts()}.csv\"";
                await _rule30.StreamCsvExportAsync(exportRequest, onlyExceptions: true, Response.Body);
                return new EmptyResult();
            }
            catch (Exception ex)
        ...
        private async Task<Rule32ValidationRequest> ResolveExportRequestAsync(Rule32ValidationSummary summary)
        {
            ...
            if (string.IsNullOrWhiteSpace(summary.TableName) || string.IsNullOrWhiteSpace(summary.ErrorTypeColumn))
                throw new InvalidOperationException("Run Rule 30 first before downloading results.");
